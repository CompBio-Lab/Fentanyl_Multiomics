...
rna <- merge(r3_RNA, y = c(r4_RNA, r15_RNA, r16_RNA), add.cell.ids = c("r3", "r4", "r15", 
"r16"), project = "RNA")
...
before_filter <- table(Idents(rna))

# Step 1: Obtain percentage of mitochondrial genes (rna) and store it in meta-data as well as nucleosome signal and TSS enrichement score (atac)
rna[["percent.mt"]] <- PercentageFeatureSet(rna, pattern = "^Mt-")

DefaultAssay(rna) <- "ATAC"  

rna <- NucleosomeSignal(rna)
rna <- TSSEnrichment(object = rna)

# Step 2: Visualize violin plot of nFeature_ATAC, nFeature_RNA, and percent.mt 
VlnPlot(rna, features = c("nFeature_RNA", "nFeature_ATAC", "percent.mt", "TSS.enrichment", "nucleosome_signal"), ncol = 5)
...
## Save Seurat object 
```{r}
saveRDS(rna, file = here::here("Data", "GEX_ATAC.rds"))
```
