...
atac_ga <- readRDS(here::here("Data", "ATAC_signac.rds"))

# set the default assay to RNA
DefaultAssay(atac_ga) <- "RNA"

# remove the ATAC assay and only keep RNA 
atac_ga[["ATAC"]] <- NULL

# convert to sce 
sce <- Seurat::as.SingleCellExperiment(atac_ga)
```
...
pb_mds <- pb_mds + geom_text(aes(label = cluster_id))

pb_mds <- pb_mds + 
  geom_line(aes(group = cluster_id), color = "grey", linetype = "dashed") + ggtitle("Pseudobulk-level multidimensional scaling (MDS) plot for gene activity")
...
top_ga %>%
  ...
  theme_minimal() +
  labs(
    title = "Volcano Plot (Gene Activity)",
    x = "Log Fold Change",
    y = "-Log10 P-value",
    color = "Cluster"
  )
...
## Save top table 
```{r}
saveRDS(top_ga, file = here::here("Data", "top_ga.rds"))
```
