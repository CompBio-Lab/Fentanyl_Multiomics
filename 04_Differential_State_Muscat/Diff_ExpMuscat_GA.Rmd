...
atac_ga <- readRDS(here::here("Data", "GEX_ATAC_GA.rds"))

# extract GA counts 
ga_counts <- GetAssayData(atac_ga, assay = "SCT", layer = "counts")

# extract metadata 
metadata = atac_ga@meta.data

# create sce object 
sce <- SingleCellExperiment(
  assays = list(counts = ga_counts),
  colData = metadata,
  metadata = metadata
)
...
df %>% 
  ...
  scale_x_log10() +
  theme_classic() +
  facet_wrap(~cluster_id) + ggtitle("Adjusted P-values for cell types (GA)")
```
...
topfil_GA <- do.call(rbind, tbl_fil)
top_GA <- do.call(rbind, tbl)
...
## Save top table 
```{r}
saveRDS(topfil_GA, file = here::here("Data", "topfil_GA.rds"))
