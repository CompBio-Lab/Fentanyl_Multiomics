...
ggVennDiagram(cb_gran_glut) + scale_fill_gradient(low="grey90",high = "red")
```

```{r}
int <- Reduce(intersect, cb_gran_glut)

rbind(top_gex, top_ga) %>% 
...
ggVennDiagram(cbx_purkinjie) + scale_fill_gradient(low="grey90",high = "red")
```
```{r}
int <- Reduce(intersect, cbx_purkinjie)

rbind(top_gex, top_ga) %>% 
  dplyr::filter(gene %in% int & cluster_id == "CBX Purkinje Gaba") %>% 
  ggplot(aes(y = gene, x = logFC, fill = dataset)) +
  geom_bar(stat="identity", position = "dodge")
