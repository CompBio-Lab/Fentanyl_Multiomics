...
## Save rds 
```{r}
saveRDS(atac, file = here::here("Data", "atac_DE.rds"))
```


# Transfer cell labels from GEX seurat object 
...
## Create column in metadata called celltype 
```{r}
# Extract active identities
cell_types <- as.character(rna@active.ident)

# Add the cell types as metadata
...
## Add cell types to metadata in atac 
```{r}
atac <- AddMetaData(object = atac, metadata = predicted.labels)
```

## Save atac signac rds 
```{r}
