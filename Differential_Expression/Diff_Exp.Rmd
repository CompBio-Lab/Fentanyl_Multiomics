...
summary(lm(Freq ~ Var1*Var2, data = cc))
...
## Save top table 
```{r}
saveRDS(top, file = here::here("Data", "top.rds"))
```

## Plot DEGs for all cell types for each interaction  
```{r}
top %>% filter(adj.P.Val < 0.01) %>% 
...
```{r}
plot1 <- EnhancedVolcano(Chol_Isl2_Gata6_4,
                         lab = Chol_Isl2_Gata6_4$gene,
                         x = 'logFC',
                         y = 'P.Value',
                         title = "Chol_Isl2_Gata6_4")
